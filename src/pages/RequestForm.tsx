...
const RequestForm = observer(() => {
  ...
  useEffect(() => {
    myStore
      .getRequestFromApi("/reg_service/api/v1/request/" + myStore.requestId)
      .then((req) => req.data)
      .then((data) => {
  ...
  const onSubmit = (data: any) => {
    ...
    if (clickedButton === "saveButton") {
      myStore
        .updateRequest("/reg_service/api/v1/request", data)
        .then(() => navigate("/"));
    } else {
      myStore.setRegisteredReq(data.id);
      myStore
        .registrationRequest("reg_service/api/v1/request/registration", data)
    ...
    <div className="content-container">
      ...
      <ThemeProvider theme={theme}>
        <form className="form-container" onSubmit={handleSubmit(onSubmit)}>
          ...
          />
          <SelectCity title="Город" register={register} setValue={setValue} />
          <SelectBrands
          ...
          <Button
            type="submit"
            variant="contained"
            onClick={() => handleClick("saveButton")}
          >
            Сохранить
          </Button>
          <Button
            type="submit"
            variant="contained"
            onClick={() => handleClick("registerButton")}
          >
